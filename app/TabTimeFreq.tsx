...
import { Zap, Lock, Unlock, Sun, Mic, Activity, Layers } from "lucide-react";

/* ── STRICT DATA INTERFACES ── */
interface TData { t: number; s: number; }
interface PrismFData { k: number; red: number; orange: number; yellow: number; green: number; blue: number; violet: number; ambient: number; }
interface VoiceFData { k: number; mag: number; phase: number; }

/* ── DATA GENERATORS ── */
function getPrismT(): TData[] {
  return Array.from({ length: 200 }, (_, i) => {
    const t = i / 200;
    const s = Math.sin(2 * Math.PI * 2 * t) + Math.sin(2 * Math.PI * 4 * t) + 
              Math.sin(2 * Math.PI * 6 * t) + Math.sin(2 * Math.PI * 8 * t) + 
              Math.sin(2 * Math.PI * 10 * t) + Math.sin(2 * Math.PI * 12 * t);
    return { t: +t.toFixed(4), s: +s.toFixed(4) };
  });
}

function getPrismF(): PrismFData[] {
  return Array.from({ length: 15 }, (_, k) => {
    const isColor = k === 2 || k === 4 || k === 6 || k === 8 || k === 10 || k === 12;
    return {
      k,
      red: k === 2 ? 1 : 0,
      orange: k === 4 ? 1 : 0,
      ...
      blue: k === 10 ? 1 : 0,
      violet: k === 12 ? 1 : 0,
      ambient: isColor ? 0 : 0.05
    };
  });
}

function getVoiceT(): TData[] {
...
function BlindOverlay({ text, subtext }: { text: string; subtext: string }) {
  return (
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center rounded-xl gap-3 shadow-2xl"
      ...
      <div className="text-center font-mono leading-6 px-8">
        <span className="text-rose-400 font-bold text-sm block tracking-widest">{text}</span>
      ...
      <div className="bg-slate-900/80 border border-slate-700/80 rounded-2xl p-6 shadow-xl flex flex-col gap-6">
        <div className="flex gap-4 p-1.5 bg-slate-950/50 rounded-xl border border-slate-800">
          <button onClick={() => { setScenario(1); setDftOn(false); }}
            ...
            <Sun size={18} className={scenario === 1 ? "text-cyan-400" : ""} />
          ...
          <button onClick={() => { setScenario(2); setDftOn(false); }}
            ...
            <Mic size={18} className={scenario === 2 ? "text-cyan-400" : ""} />
        ...
        <div className="grid grid-cols-2 gap-6 animate-in fade-in zoom-in-95 duration-500">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5 shadow-xl flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
              ...
              <span className="font-mono text-xs text-white tracking-wider">{"COMPOSITE WAVEFORM s(t) (WHITE LIGHT)"}</span>
            </div>
            {/* @ts-ignore */}
            <ResponsiveContainer width="100%" height={320}>
              ...
              <LineChart data={prismTd}>
                ...
                {/* @ts-ignore */}
                <Tooltip contentStyle={{ backgroundColor: "#0f172a", border: "1px solid #fff", borderRadius: 8, color: "#fff", fontFamily: "monospace" }} />
                {/* @ts-ignore */}
                <Line type="monotone" dataKey="s" stroke="#ffffff" dot={false} strokeWidth={2.5} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-5 shadow-xl flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
              <Sun size={15} className={dftOn ? "text-fuchsia-400" : "text-slate-600"} />
            ...
            <ResponsiveContainer width="100%" height={320}>
              ...
              <BarChart data={prismFd} barCategoryGap="20%">
                ...
                <YAxis tick={{ fill: "#475569", fontSize: 10 }} tickLine={false} />
                {/* @ts-ignore */}
                <Tooltip contentStyle={{ backgroundColor: "#0f172a", border: "1px solid #d946ef", borderRadius: 8, fontFamily: "monospace" }} />
                {/* @ts-ignore */}
                <Bar dataKey="red" stackId="a" fill="#ef4444" isAnimationActive animationDuration={600} />
                {/* @ts-ignore */}
                <Bar dataKey="orange" stackId="a" fill="#f97316" isAnimationActive animationDuration={600} />
                {/* @ts-ignore */}
                <Bar dataKey="yellow" stackId="a" fill="#eab308" isAnimationActive animationDuration={600} />
                {/* @ts-ignore */}
                <Bar dataKey="green" stackId="a" fill="#22c55e" isAnimationActive animationDuration={600} />
                {/* @ts-ignore */}
                <Bar dataKey="blue" stackId="a" fill="#3b82f6" isAnimationActive animationDuration={600} />
                {/* @ts-ignore */}
                <Bar dataKey="violet" stackId="a" fill="#a855f7" isAnimationActive animationDuration={600} />
                {/* @ts-ignore */}
                <Bar dataKey="ambient" stackId="a" fill="#1e293b" isAnimationActive animationDuration={600} />
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="col-span-5 bg-slate-900 border border-slate-700 rounded-2xl p-5 shadow-xl flex flex-col gap-4">
            ...
            <ResponsiveContainer width="100%" height={480}>
              ...
              <LineChart data={voiceTd} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                {/* @ts-ignore */}
                <Tooltip contentStyle={{ backgroundColor: "#0f172a", border: "1px solid #22d3ee", borderRadius: 8, fontFamily: "monospace" }} />
                {/* @ts-ignore */}
          ...
          <div className="col-span-7 relative bg-slate-900 border border-slate-700 rounded-2xl flex flex-col shadow-xl overflow-hidden">
            <div className="flex-1 p-5 border-b border-slate-800 flex flex-col gap-3">
              <div className="flex items-center gap-2">
                <Layers size={14} className={dftOn ? "text-amber-400" : "text-slate-600"} />
            ...
            <div className="flex-1 p-5 flex flex-col gap-3">
              <div className="flex items-center gap-2">
                <Layers size={14} className={dftOn ? "text-indigo-400" : "text-slate-600"} />
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                <ScatterChart margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  {/* @ts-ignore */}
                  <YAxis dataKey="phase" type="number" tick={{ fill: "#475569", fontSize: 9 }} tickLine={false} domain={[-3.15, 3.15]} ticks={[-3.14, 0, 3.14]} />
                  {/* @ts-ignore */}
                  <Tooltip cursor={{ strokeDasharray: "3 3" }} contentStyle={{ backgroundColor: "#0f172a", border: "1px solid #818cf8", borderRadius: 8, fontFamily: "monospace" }} />
                  {/* @ts-ignore */}
